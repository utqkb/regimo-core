...
# =============================================================================
def sanitize_dataset_record(record: dict) -> dict:
    """Fills missing identifiers with fallback values to prevent Cypher ingestion drops."""
    if not record.get("name"):
        record["name"] = f"unnamed_dataset_{record.get('id', 'unknown')}"
    
    if "licenses" in record and isinstance(record["licenses"], list):
        for lic in record["licenses"]:
            if isinstance(lic, dict) and not lic.get("name"):
                lic["name"] = "unspecified_license"
                
    if "resources" in record and isinstance(record["resources"], list):
        for res in record["resources"]:
            if isinstance(res, dict) and not res.get("path"):
                res["path"] = "undefined_resource_path"
...
def ingest_oep_metadata(driver: Driver, record: dict[str, Any], success_count, failed_count, ingest_errors):
    ...
    """
    record = sanitize_dataset_record(record)
