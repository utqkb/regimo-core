...
def verify_neo4j_connection(neo_driver: Driver):
    ...
    try:
        ...
        print(
            f"Failed to connect or verify Neo4j connection. Ensure the database is running and credentials are correct: {e}"
        )
...
def clear_database(driver: Driver):
    ...
    confirmation = (
        input("Do you really want to delete the database? (yes/no): ").strip().lower()
    )
...
# =============================================================================
def ingest_oep_metadata(
    driver: Driver, record: dict[str, Any], success_count, failed_count, ingest_errors
):
    """
    ...
    cypher_query = """
    CALL apoc.do.when(
        $dataset.`@id` IS NOT NULL AND trim($dataset.`@id`) <> "",
        '
    ...
    WITH value.d AS d, $dataset.metaMetadata AS mm
        ...
        try:
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                raw_data: list[dict[str, Any]] = json.loads(
                    json_content
                )  # Converts e.g. "@id": null to "@id": None -> converts everything to Python syntax so Python can work with it (since null would cause errors, as it's called none in Python, not null)
...
if __name__ == "__main__":
    ...
    print(
        "3 - Use existing local metadata files (from previous Downloads) and compare them to the actual OEP Metadata"
    )
    ...
    if choice == "1":
        ...
        records_to_ingest = validate_directory(METADATA_DIR, LINKML_SCHEMA_PATH)
        ...
        for table_name in table_names:
            try:
                log.info(
                    f"Downloading Metadata for table: {table_name}, "
